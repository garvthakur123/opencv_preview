...
template <unsigned int SIZE> __global__ void sobelKernel(const unsigned char*inImg,unsigned char*outImg,
...
__host__ void SobelFilter::operator()(const unsigned char *input,unsigned char *output,
	...
	sobelKernel<BSIZE><<<this->grid,this->threads>>>(dInput,dOutput,w,h,c);
